...
from zunda_w import edit, silent, voice_vox, SpeakerUnit, transcribe_non_silence_srt, transcribe_with_config, file_hash, \
    SpeakerCompose, merge
...
class Options:
    ...
    def voicevox_profiles(self, n: int) -> VoiceVoxProfiles:
        if self.v_profile_json and os.path.exists(self.v_profile_json) and try_json_parse(self.v_profile_json):
            return json.loads(Path(self.v_profile_json).read_text(encoding='UTF-8'))
        else:
    ...
    try:
        ...
        for idx, (original_audio, speaker_id) in enumerate(zip(audio_files, speakers)):
            ...
            # オリジナル音声の無音区間を切り抜き
                ...
                # 切り抜き音声から文字おこし
                stt_file = transcribe_non_silence_srt(silent_audio, meta, arg.whisper_profile, original_audio,
                                                      cache_dir)
